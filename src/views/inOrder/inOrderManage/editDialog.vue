            ...
            <el-dialog title="编辑" :visible.sync="dialogVisible" width="1000px" :close-on-click-modal="false">
                <div>
                    <div>
                        <div>单据明细</div>
                        <div>
                            <el-form ref="editForm" :model="form" :rules="rules" size="small" :inline="true"
                                label-width="80px">
                                ...
                                <el-form-item label="入库时间" prop="stockInTime">
                                    <el-date-picker v-model="form.stockInTime" type="date" :picker-options="pickerOptions"
                                        clearable placeholder="选择日期" value-format="yyyy-MM-dd" />
                                </el-form-item>
                            </el-form>
                                ...
                                <el-table-column prop="materialName" width="150" label="物料">
                                    <template slot-scope="scope">
                                        <span v-if="scope.row.isEdit">
                                            ...
                                            <el-select v-model="scope.row.tempMaterialId" filterable placeholder="请选择"
                                                @change="materialChange(scope.row);">
                                                <el-option v-for="item in materialList" :key="'material-' + item.id"
...
import { edit } from "@/api/inOrder/inOrderManage"
    ...
    methods: {
        ...
        save() {
            ...
            this.$refs['editForm'].validate((valid) => {
                ...
                let tempOrderItem = deepClone(this.orderItems);
                tempOrderItem.forEach((item) => {
                    if (item.isNew) {
                        delete item.id
                    }
                })
